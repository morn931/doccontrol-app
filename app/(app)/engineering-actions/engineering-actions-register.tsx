    ...
    <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4">
      <div className="w-full max-w-xl rounded-xl bg-white p-5 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3 text-sm">
          <div><label className="block text-slate-600 mb-1">Decision title *</label><input value={f.title} onChange={set('title')} placeholder="Short one-line subject — e.g. “33kV busbar arrangement at Main Consumer Substation”" className="w-full rounded-md border border-slate-300 p-2" /></div>
          <div><label className="block text-slate-600 mb-1">Description / background</label><textarea value={f.background} onChange={set('background')} rows={2} placeholder="What triggered the decision — the problem, constraint or question being resolved." className="w-full rounded-md border border-slate-300 p-2" /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-slate-600 mb-1">Discipline</label><input value={f.discipline} onChange={set('discipline')} placeholder="Electrical / I&C / Civil / Mechanical" className="w-full rounded-md border border-slate-300 p-2" /></div>
            <div><label className="block text-slate-600 mb-1">Document no.</label><input value={f.documentNumber} onChange={set('documentNumber')} placeholder="SLD / drawing / calc ref, e.g. 6105AK132-…" className="w-full rounded-md border border-slate-300 p-2" /></div>
          </div>
          <div><label className="block text-slate-600 mb-1">Options considered</label><textarea value={f.optionsConsidered} onChange={set('optionsConsidered')} rows={2} placeholder="Alternatives evaluated — e.g. (1) Single bus, NC tie; (2) Split bus, NO tie with auto transfer" className="w-full rounded-md border border-slate-300 p-2" /></div>
          <div><label className="block text-slate-600 mb-1">Proposed decision</label><textarea value={f.decisionMade} onChange={set('decisionMade')} rows={2} placeholder="The decision / outcome selected (leave blank if still to be decided by the approver)." className="w-full rounded-md border border-slate-300 p-2" /></div>
          <div><label className="block text-slate-600 mb-1">Rationale</label><textarea value={f.rationale} onChange={set('rationale')} rows={2} placeholder="Why this option over the alternatives — cost, safety, fault level, schedule, standards…" className="w-full rounded-md border border-slate-300 p-2" /></div>
          <div className="grid grid-cols-2 gap-3">
