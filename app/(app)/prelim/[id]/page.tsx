...
export default async function PrelimSessionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: docs } = await db.from('prelim_document')
    .select('id, document_number, revision, title, discipline, document_type, source_file_name, working_file_name, cddl_doc_id, markup_comments, markup_layer, markup_committed_at, outcome, outcome_note, outcome_by_email, outcome_at, rework_to_email, handed_over_batch_id, handed_over_at, pulled_by_email, created_at')
