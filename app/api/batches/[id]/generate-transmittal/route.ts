...
async function buildTransmittalPdf(data: TransmittalData): Promise<Buffer> {
  ...
  return new Promise<Buffer>((resolve, reject) => {
    ...
    for (let p = 0; p < range.count; p++) {
      pdf.switchToPage(p)
      // Drop the bottom margin while writing the footer — it sits below the margin, and
      // otherwise PDFKit spawns a blank page for each footer (the "empty pages" bug).
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!can(__perms, FK.ACTION_GENERATE_TRANSMITTAL, (profile?.role ?? 'reviewer') as any))
