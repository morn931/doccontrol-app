...
export async function importWorkbook(
  ...
  for (const [key, incoming] of awarded) {
    ...
    const merged: any = { id: prev.id, is_awarded: true, normalized_document_number: key }
    for (const f of ENTRY_FIELDS) merged[f] = mergeField(prev[f], incoming[f])
    merged.source_register_id = incoming.source_register_id
    merged.source_types = [...new Set([...(prev.source_types ?? []), registerType])]
    merged.source_type  = [...merged.source_types]
      .sort((a, b) => (SOURCE_RANK[b] ?? -1) - (SOURCE_RANK[a] ?? -1))[0]
    merged.raw = { ...(prev.raw ?? {}), ...(incoming.raw ?? {}) }
    if (incoming.vendor_id) merged.vendor_id = incoming.vendor_id
    if (incoming.progress_source) merged.progress_source = incoming.progress_source
  ...
  for (const part of chunk(toUpsert, 500)) {
    const { error } = await db.from('mddr_entries').upsert(part)
    if (error) errors.push(`Update: ${error.message}`); else updated += part.length
  }
