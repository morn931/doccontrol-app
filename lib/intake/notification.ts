 ...
 *
 * Layout: navy header (what + where it came from) → one card per document with the number,
 * title and an overall badge → two side-by-side boxes, green "Correct" vs red "Needs
 * attention", with the findings listed under each → the AI's note.
 */
import type { AiReview } from './ai-review'

const CF = {
  navy: '#0b2a5b', navySub: '#a9c0ea', ink: '#1f2937', muted: '#6b7280', line: '#e5e7eb',
...
function overallPill(overall: AiReview['overall']): string {
  ...
  return `<span style="display:inline-block;padding:4px 12px;border-radius:999px;background:${bg};color:${fg};font-size:12px;font-weight:700;white-space:nowrap">${label}</span>`
}
...
function findingsBox(title: string, items: string[], kind: 'good' | 'bad'): string {
  ...
  const body = items.length
    ? `<ul style="margin:8px 0 0;padding-left:18px">${items.map((i) => `<li style="margin:3px 0;color:${CF.ink};font-size:13px;line-height:1.4">${i}</li>`).join('')}</ul>`
    : `<div style="margin-top:8px;color:${CF.muted};font-size:13px">${kind === 'good' ? '&mdash;' : 'Nothing flagged.'}</div>`
  return `<td valign="top" width="50%" style="padding:0 5px">
    <div style="border:1px solid ${ln};background:${bg};border-radius:8px;padding:12px 14px;height:100%">
      <div style="color:${fg};font-weight:700;font-size:13px;text-transform:uppercase;letter-spacing:.03em">${title}</div>${body}
    </div></td>`
}
...
function docCard(fileName: string, r: AiReview | null): string {
  if (!r) {
    return `<div style="border:1px solid ${CF.line};border-radius:10px;padding:14px;margin:0 0 14px">
      <div style="font-weight:700;color:${CF.navy}">${esc(fileName)}</div>
      <div style="color:${CF.muted};font-size:13px;margin-top:4px">AI review unavailable for this document.</div></div>`
  }
  const x = r.extracted
  ...
  return `<div style="border:1px solid ${CF.line};border-radius:10px;margin:0 0 16px;overflow:hidden">
    <div style="padding:14px 16px;border-bottom:1px solid ${CF.line};background:#fbfcfe">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>
        <td valign="top">
          <div style="font-size:15px;font-weight:700;color:${CF.navy};letter-spacing:.01em">${esc(fileName.replace(/\.pdf$/i, ''))}</div>
          <div style="font-size:13px;color:${CF.ink};margin-top:3px">${esc(x.title)}</div>
        </td>
        <td valign="top" align="right" style="white-space:nowrap;padding-left:10px">${overallPill(r.overall)}</td>
      </tr></table>
      ${meta ? `<div style="font-size:12px;margin-top:8px">${meta}</div>` : ''}
    </div>
    <div style="padding:12px 11px">
      ...
      ${r.checks.notes ? `<div style="font-size:12px;color:${CF.muted};font-style:italic;padding:10px 6px 2px;line-height:1.4">${esc(r.checks.notes)}</div>` : ''}
      ${!r.validation.length ? `<div style="font-size:11px;color:${CF.muted};padding:2px 6px">No SDDR on record for this vendor — extraction only (no field-by-field comparison).</div>` : ''}
    </div>
  </div>`
}
...
export function buildNotificationEmail(
  ...
  return `<div style="font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;max-width:680px;margin:0 auto;color:${CF.ink};background:#f4f6fb;padding:18px">
    <div style="background:${CF.navy};border-radius:12px 12px 0 0;padding:18px 20px">
      <div style="font-size:11px;letter-spacing:.12em;color:${CF.navySub};text-transform:uppercase">CoreDocs &middot; Vendor Document Pre-Review</div>
      <div style="font-size:19px;font-weight:800;color:#ffffff;margin-top:3px">${results.length} document${results.length === 1 ? '' : 's'} received &mdash; ${esc(packageCode)}</div>
      <div style="font-size:13px;color:${CF.navySub};margin-top:3px">From ${esc(vendorName || packageName || packageCode)} &middot; auto-reviewed by AI</div>
    </div>
    <div style="background:#ffffff;border:1px solid ${CF.line};border-top:none;border-radius:0 0 12px 12px;padding:16px">
      ${cards}
      <div style="font-size:12px;color:${CF.muted};padding:6px 4px 0;border-top:1px solid ${CF.line};margin-top:4px;padding-top:10px">
